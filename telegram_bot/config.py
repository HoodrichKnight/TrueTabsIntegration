...
if not BOT_TOKEN:
    print("Ошибка: Токен бота не найден. Установите переменную окружения BOT_TOKEN.", file=sys.stderr)
    # Можно добавить sys.exit(1) если токен обязателен для запуска

# Дополнительная проверка наличия путей (опционально, но полезно)
...
# Убедимся, что директория для временных файлов и БД существуют
