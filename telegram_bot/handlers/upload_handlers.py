...
SOURCE_PARAMS_ORDER = {
    ...
    "mongodb": ["source_url", "mongo_db", "mongo_collection"], # source_url для MongoDB это URI
...
async def process_source_input_method(callback: CallbackQuery, state: FSMContext):
    ...
    if method == 'manual':
        ...
        if initial_param_state and first_param_key:
...
async def process_pg_url_manual(message: Message, state: FSMContext):
    ...
    await move_to_next_source_param_or_tt(message, state)
...
# TODO: Адаптировать обработчики ручного ввода для всех остальных источников

async def move_to_next_source_param_or_tt(message: Message, state: FSMContext):
    state_data = await state.get_data()
    param_keys_order = state_data.get('param_keys_order', [])
    current_param_index = state_data.get('current_param_index', 0)
    ...
    if next_param_index < len(param_keys_order):
        ...
        await state.set_state(UploadProcess.select_tt_input_method)
        await message.answer("Выберите способ ввода параметров для True Tabs:", reply_markup=select_input_method_keyboard('tt'))
...
async def process_upload_token_manual(message: Message, state: FSMContext):
    ...
    await move_to_next_tt_param_or_confirm(message, state)
...
async def move_to_next_tt_param_or_confirm(message: Message, state: FSMContext):
    state_data = await state.get_data()
    param_keys_order = state_data.get('param_keys_order', [])
    current_param_index = state_data.get('current_param_index', 0)
    ...
    if next_param_index < len(param_keys_order):
        ...
        next_param_key = param_keys_order[next_param_index]
        next_state = None
        if next_param_key == "upload_datasheet_id":
             next_state = UploadProcess.waiting_datasheet_id
        elif next_param_key == "upload_field_map_json":
             next_state = UploadProcess.waiting_field_map_json

        if next_state:
             await state.set_state(next_state)
             await message.answer(f"Введите параметр '{next_param_key}':")
        else:
             await message.answer("Ошибка определения следующего параметра TT.")
             await state.clear()
...
async def handle_confirm_upload(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    for key, value in source_params.items():
        if value is not None:
            ...
            # TODO: Особая обработка типов, если Rust ожидает не строки (булевы, числа) - сейчас все как строки
...
async def process_upload_task(bot: Bot, chat_id: int, rust_args: list, source_type: str, datasheet_id: str, output_filepath: str):
    ...
    # TODO: Парсить более детальный структурированный вывод из result.message (если Rust будет его давать)
